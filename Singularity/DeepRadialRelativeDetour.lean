...
/-!
# Relative detour estimates for all deep radial endpoints

The unrestricted deep-domain lower bound removes the fixed-axis-tube assumption
from the relative detour estimate. Endpoint distance is still bounded by a
fixed multiple of the detour radius, as needed by the shrinking-ball argument.
-/
...
/-- Uniform relative detour error for all pairs above one radial depth buffer. -/
theorem cocompact_deep_radial_relative_green_detour
    ...
    ∃ H R₀ : ℝ, 0 < H ∧ ∀ R ≥ R₀, ∀ (q : SL(2, ℝ)) (r : ℝ) (u x y : Γ),
  ...
  refine ⟨H,R₀,hH,?_⟩
  intro R hR q r u x y hx hy hdist hdiam hexcess
  have hl : α * Real.exp (-β*(B*R)) ≤
      killedGreen s μ (radialOrbitSublevel Γ q UpperHalfPlane.I r) x y := by
    apply le_trans _ (hlower q r x y hx hy)
    exact mul_le_mul_of_nonneg_left
      (Real.exp_le_exp.mpr (mul_le_mul_of_nonpos_left hdiam (neg_nonpos.mpr hβ.le))) hα.le
  calc
